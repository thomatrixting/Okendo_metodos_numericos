//  g++-14 -std=c++17 particle.cpp main_md.cpp
#include "particle.h"
#include "integrator.h"
#include "colider.h"
#include "boundary.h"
#include <vector>
#include <fstream>

#include <map>


void print_to_csv(const std::vector<Particle> &, int istep);

int main(int argc, char **argv) {
  std::vector<Particle> bodies;
  bodies.resize(1); // only one particle for now


  // parameters
  std::map<std::string, double> p;
  p["T0"] = 0.0;
  p["TF"] = 100.8767;
  p["DT"] = 0.1;
  p["G"] = 0.0; //-9.81;

  // Force collider
  Collider collider(p);

  // Time initialization
  TimeIntegrator integrator(p["DT"]);

  // Boundary conditions
  Boundary bc(2.345, 0.0, 0.0, 0.0, 1.0); // RMAX, CX, CY, CZ, EN

  // initial conditions and properties
  bodies[0].R[2] = 0.987;  // z is upwards, x to the right
  bodies[0].V[0] = 4.9876;//12.987; // z is upwards, x to the right
  bodies[0].V[2] = 0.0; //4.9876; //3.987; // z is upwards, x to the right
  bodies[0].rad  = 0.103;
  bodies[0].mass = 0.337;
  collider.computeForces(bodies); // force at t = 0
  integrator.startIntegration(bodies); // start integration algorithm

  // Time iteration
  const int niter = int((p["TF"] - p["T0"])/p["DT"]);
  for(int ii = 1; ii < niter; ++ii) {
    collider.computeForces(bodies);
    integrator.timeStep(bodies);
	  bc.apply(bodies);
    print_to_csv(bodies,ii);

  }

  return 0;
}

void print_to_csv(const std::vector<Particle> & particles, int istep){

  std::string index = std::to_string(istep);
  int nfields = 7;
  
  std::string filename = index;

    nfields -= index.size();  // FIXED: Using -= instead of =-

    if (nfields > 0) {
        filename.insert(0, nfields, '0');  // Add leading zeros
    }

  filename = "./data/" + filename + ".csv";
  std::ofstream file(filename);

  file << "mass, rad, R_x, R_y, R_z, V_x, V_y, V_z, F_x, F_y, F_z" << "\n";

  for (auto & p: particles){

    file << p.mass << "," << p.rad << ","
        << p.R[0] << "," << p.R[1] << "," << p.R[2] << ","
        << p.V[0] << "," << p.V[1] << "," << p.V[2] << ","
        << p.F[0] << "," << p.F[1] << "," << p.F[2] << ","
        << "\n";


  }
  file.close();


}